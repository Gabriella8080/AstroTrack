...
def plot_doppler_shifts(
    ...
    Parameters:
         ...
         time_window (float): Half-width of x-axis in around visibility center [min].
    ...
    for satellite_data in all_satellite_data:
        ...
        velocity_r = np.array(satellite_data["Velocities"])  # relative velocity in km/s
        ...
        relative_time = (epochs_np - center_time) / np.timedelta64(1, "m")  # minutes
...
def check_doppler_resolution(
    ...
    """Check if Doppler shift can be resolved, given experiment's frequency resolution.
    ...
    for satellite_data in all_satellite_data:
        ...
        for f0 in f0_array:
            ...
            row[
                f"{f0/1e6:.1f} MHz"
            ] = f"{max_doppler_khz:.2f} kHz | {'YES' if max_doppler_khz > resolution/1e3 else 'NO'}"
